...
class RidgeLoss:
      ...
      def fit(self, d, y, W):
          d_y = tf.multiply(tf.norm(tf.subtract(d, y)), 0.5)
          alpha_W = tf.multiply(tf.multiply(tf.norm(W), self.alpha), 0.5)
...
def stlr(W, x):
    ...
    # W size (100,100), x size (x_batch_size,28,100)
    x_W = tf.reshape(x, [x_batch_size,input_dim,100,1]) * W
    sum = tf.reshape(tf.matmul(x, W), [x_batch_size,input_dim,1,100]) - x_W
    
    # I_spsum:空間合計　(x_batch_size,input_dim,100,100) * (x_batch_size,input_dim,1,100) = (x_batch_size,input_dim,100,100)
    I_spsum = x_W * sum
    
    # I_sum size (100,100)
    I_sum = tf.reduce_sum(I_spsum, [0, 1])
    
    # I_sumの小さい方から33%と66%の値を取得
    I_sum_array = np.percentile(I_sum, q=[33,66])
    
    # 条件ごとにI_sumを更新するためにnumpyに変換
    I_sum = I_sum.numpy()
    I_sum = np.where(I_sum <= I_sum_array[0], I_sum - I_sum_array[0], I_sum)
    I_sum = np.where(I_sum >= I_sum_array[1], I_sum - I_sum_array[1], I_sum)
    # tensorに戻す
    I_sum = tf.constant(I_sum)
    
    # (I_sum_array[0] < I_sum < I_sum_array[1])を満たす要素を0にするための係数行列
    tensor_condition = np.zeros((100,100))
    tensor_condition = np.where(I_sum <= I_sum_array[0], 1, tensor_condition)
    tensor_condition = np.where(I_sum >= I_sum_array[1], 1, tensor_condition)
    # tf.float32と演算できるようにcast
    tensor_condition = tf.cast(tensor_condition, tf.float32)
    
    # dW size (100, 100)
    dW = K.sigmoid(I_sum) * tensor_condition
    #print(dW)
...
def train_step_stlr(inputs):
    
    model(inputs)
    
    # dW size (100, 100), 0.0000001:学習率
    dW = stlr(model.W_rec, model.x) * 0.0000001
    
    #print(dW)
    
    # model.W_rec size (100, 100)
    model.W_rec.assign(model.W_rec + dW)
...
train_step_stlr(u_train[0:5])
train_step_stlr(u_test[0:5])
...
def train(epochs):
  ...
  for ep in range(epochs):
    ...
    d = tf.one_hot(d, depth=10)   # y size (data_size, 10)
